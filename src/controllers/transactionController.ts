...
export default class TransactionController {
  ...
  public static purchaseCard(userId: number, cardType: number, walletId: number): Promise<Card> {
    return connection.transaction(async (t: DbTransaction) => {
      ...
      const cards = await Card.findAll({
        ...
        where: and(
          where(col("Transaction.card"), "IS", null),
          where(col("Card.type"), cardType as any)
        ),
        limit: 1,
        transaction: t,
      });
  ...
  public static increaseBallance(
    ...
    return connection.transaction(async (t: DbTransaction) => {
      ...
      if(userToIncreament.get('type') !== UserType.sellingPointId) throw new Error("[RETURN] Only Selling Points Users Allowed")
      await Transaction.create({
